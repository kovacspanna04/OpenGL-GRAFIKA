...
namespace GrafikaSzeminarium
{
    internal class Program
    {
        ...
        private static ModelObjectDescriptor roundRectangle;
        private static double barrelRadius;         // a dezsa sugara majd
        ...
        private static void GraphicWindow_Load()
        {
            ...
            // letrehozom a ket kulonbozo obiektumot a ket kulon fele teglalapnak
            rectangle = ModelObjectDescriptor.CreateRectangle(Gl, false);
            roundRectangle = ModelObjectDescriptor.CreateRectangle(Gl, true);

            barrelRadius = barrelRadiusCalculator(rectangle.RectangleWidth, 10.0);

            Gl.ClearColor(System.Drawing.Color.White);

            // hogy latszodjanak a hatso teglalapok is
        ...
        private static unsafe void GraphicWindow_Render(double deltaTime)
        {
            ...
            // kamera poziciojanak beallitasa ( a vilagitas mindig a nezo iranyabol jon)
            Vector3 lightPosition = new Vector3(
            camera.Position.X * 1.2f,       // a feny mindig a kamera mogott van
            camera.Position.Y * 1.2f,       // hogy biztos legyen, hogy kivul van
            ...
            var translation = Matrix4X4.CreateTranslation(0, 0, (float)barrelRadius);       // transzlacios matrix-> az objektumot Z iranyba tolja barrelRadiussal
            var flatTranslation = Matrix4X4.CreateTranslation(0, 3f, 0);        // fent legyen
            var roundTranslation = Matrix4X4.CreateTranslation(0, -3f, 0);      // alatta legyen

            for (int i = 0; i < 18; i++)        // letrehozom a 18 teglalapot mind2 esetben a megfelelo helyekre, a megfelelo elforgatassal
            {
                var rotation = Matrix4X4.CreateRotationY((float)(i * Math.PI / 9));     // 20 fokkal vannak egymashoz kepest elforditva
                ...
                SetModelMatrix(modelMatrix);
                DrawModelObject(roundRectangle);
            }
        ...
        public static double barrelRadiusCalculator(double lapSzelesseg, double szog)
        {
